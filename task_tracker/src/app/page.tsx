...
export default function Dashboard() {
  ...
  // Stat groups shown right-aligned in each tab's title row.
  // Pipeline stats are tinted to match their board column's header color.
  const expStats = (
    <>
      <Stat label="Total" value={experiments.length} />
      <Stat label="Implementation" value={experiments.filter((e) => e.status === 'Implementation').length} color="text-blue-600 dark:text-blue-300" />
      <Stat label="SQA" value={experiments.filter((e) => e.status === 'SQA').length} color="text-cyan-600 dark:text-cyan-300" />
      <Stat label="Code Review" value={experiments.filter((e) => e.status === 'Code Review').length} color="text-rose-600 dark:text-rose-300" />
      <Stat label="Live" value={experiments.filter((e) => e.status === 'Live').length} color="text-teal-600 dark:text-teal-300" />
      <Stat label="Winners" value={experiments.filter((e) => e.result === 'Winner').length} color="text-green-500 dark:text-green-400" />
      <Stat label="Completed" value={experiments.filter((e) => e.status === 'Completed').length} color="text-emerald-600 dark:text-emerald-300" />
    </>
  );
  const taskStats = (
    <>
      <Stat label="Total Tasks" value={tasks.length} />
      <Stat label="Active Tasks" value={tasks.filter((t) => t.status !== 'Completed').length} color="text-blue-500 dark:text-blue-400" />
      <Stat label="Completed" value={tasks.filter((t) => t.status === 'Completed').length} color="text-green-500 dark:text-green-400" />
    </>
  );
  const inboxStats = (
    <>
      <Stat label="Total Submissions" value={ideas.length} />
      <Stat label="Pending Ideas" value={pendingIdeas} color="text-violet-500 dark:text-violet-400" />
      <Stat label="Approved" value={ideas.filter((i) => i.status === 'Approved').length} color="text-green-500 dark:text-green-400" />
      <Stat label="Rejected" value={ideas.filter((i) => i.status === 'Rejected').length} color="text-rose-500 dark:text-rose-400" />
    </>
  );
  ...
  return (
    <div className="min-h-screen mesh-bg">
              ...
              >
                ...
                {t.count !== undefined && t.count > 0 && (
                  <span className={`px-1.5 py-0.5 rounded-full text-xs font-bold leading-none ${
                    t.id === 'inbox' ? 'bg-violet-500 text-white' : 'bg-slate-200 text-slate-600 dark:bg-white/10 dark:text-white/60'
                  }`}>
                    {t.count}
                  </span>
                )}
      ...
      {/* Main content */}
      <main className={`${tab === 'board' ? 'max-w-none' : 'max-w-7xl'} mx-auto px-6 py-6`}>
        {loading ? (
            ...
            {tab === 'experiments' && (
              <div className="space-y-4">
                <div className="flex items-center justify-between gap-8">
                  ...
                  <div className="flex items-center gap-6">{expStats}</div>
                </div>
                <ExperimentTable
            ...
            {tab === 'tasks' && (
              <div className="space-y-4">
                <div className="flex items-center justify-between gap-8">
                  ...
                  <div className="flex items-center gap-6">{taskStats}</div>
                </div>
                <TaskList
                  tasks={tasks}
                  onAdd={addTask}
            ...
            {tab === 'inbox' && (
              <div className="space-y-4">
                <div className="flex items-center justify-between gap-8">
                  ...
                  <div className="flex items-center gap-6">{inboxStats}</div>
                </div>
                <InboxTab
                  ideas={ideas}
...
function Stat({ label, value, color }: { label: string; value: number; color?: string }) {
  return (
    <div className="flex items-center gap-2.5">
      <span className={`text-lg font-bold tabular-nums ${color ?? 'text-slate-800 dark:text-white'}`}>
        {value}
      </span>
